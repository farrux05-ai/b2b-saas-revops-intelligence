...
**Solution:** Unified RevOps Intelligence Engine — Snowflake + dbt + Elementary + Lightdash + Dagster  
...
HubSpot + Stripe + Zendesk + PostHog
    ...
    Snowflake Warehouse (RAW_DATA schema)
         │
         ▼ dbt (transformation — 3 layers + Elementary checks)
    dim_accounts · fct_accounts_health · fct_pql_signals · fct_mrr_waterfall
         │
         ├──▶ Lightdash Semantic Layer ──▶ Slack AI Agent (alerts & Q&A)
         │
...
Any record that cannot be matched gets `match_method = 'unresolved'` and is surfaced for manual reconciliation. Unresolved accounts cannot receive Reverse ETL enrichment in HubSpot.
...
### Why Enterprise Snowflake Warehouse?

Choosing Snowflake as the single cloud data warehouse provides:
- **Centralized Data Governance:** Role-based access control (RBAC) across `RAW_DATA`, `STAGING`, and `MARTS`
- **High Concurrency & Latency:** Lightdash and Elementary query Snowflake instantly without locks
- **SQL Consistency:** No SQL dialect translation bugs between environments
...
1. **Start with a business number, not a schema.** The $87K at-risk figure unlocked CEO buy-in on Day 3. Find the pain that has a dollar value.
2. **Explainable > accurate.** The 3-signal health model is not complex, but CS managers understand exactly why an account is flagged. Black-box scores create hesitation.
3. **Tests = organizational trust.** With 160 dbt tests running daily, the CFO stopped asking *"is this number right?"*
4. **Reverse ETL closes the loop.** Pushing data back into HubSpot made insights actionable without changing human behavior.
...
- **[Deployment Runbook](DEPLOYMENT.md)** — Snowflake setup, Lightdash, CI/CD, Dagster scheduling
