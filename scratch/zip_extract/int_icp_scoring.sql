...
--
-- O'ZGARISH:
--   int_finance_aggregated → int_billing_aggregated
--
-- MAS'ULIYAT:
--   Account darajasida ICP (Ideal Customer Profile) Fit Score hisoblaydi.
--   industry + account_segment + revenue → icp_score, icp_tier.
--   fct_pql_signals da GTM Priority Matrix uchun ishlatiladi.
-- =============================================================================
...
-- int_billing_aggregated: workspace_id darajasida MRR
billing as (
    select
        workspace_id,
        total_mrr
...
scoring_base as (
    select
        ...
        -- Segmentatsiya: ARR asosida (int_icp_scoring da, int_accounts_integrated dan oldin)
        case
...
scoring as (
    select
        ...
        -- 1. Industry Fit (seeds/icp_industry_scores.csv)
        coalesce(ind.industry_score, 5)                 as industry_score,

        -- 2. Segment Fit (seeds/icp_segment_scores.csv)
